...
use crate::{Address, name::Name, hardware_integration::{TimeDriver, TimeDriverTrait}, CanMessage, ParameterGroupNumber, CanNetworkManager, CanPriority};
	...
	pub fn update(&mut self, network_manager: &CanNetworkManager) {
		...
		match self.current_state {
			...
			State::WaitForRequestContentionPeriod => {
				...
				if TimeDriver::time_elapsed() - self.timestamp >= Duration::from_millis(250) + self.random_claim_delay {
					// After the wait, check if our address has been claimed.
					let other_name: Option<Name> = None;
